...
def parse_doc(doc):
    ...
    if intro:
        ...
        if len(content) > 0:
            out["title"] = content[0]
            if len(content) > 1:
                out["abstract"] = content[1].replace("\n    ", " ")
...
def wrapped_partial(func, suggest: dict = None, **prescribe):
    suggest = suggest or {}
    kwargs = dict(**prescribe, **suggest)
    sign = signature(func)
    ...
    expected = list(suggest.items())
    pfunc = partial(func, **kwargs)

    # This a modified copy of boltons.funcutils.wraps
    fb = FunctionBuilder.from_func(func)
    for arg in injected:
        fb.remove_arg(arg)
    for arg, default in expected:
        fb.add_arg(arg, default, kwonly=True)
    fb.body = f"return _call({fb.get_invocation_str()})"

    execdict = dict(_call=pfunc, _func=func)
    partial_func = fb.get_func(execdict, with_dict=True)
    partial_func.__dict__["_partial"] = True
    return partial_func
